...
const EventCharacterElement: React.FC<{
  studioId: StudioId
  ...
  const character = useLiveQuery(
    ...
    [characterId, aliasId, transform, styles],
    undefined
  )
  ...
  useEffect(() => {
    async function getCharRef() {
      ...
      const foundCharacterReference = await getCharacterReference(
        studioId,
        character.id,
        aliasId
      )

      if (!foundCharacterReference) return

      setCharacterDisplayFormat(
        getCharacterRefDisplayFormat(
          foundCharacterReference,
          transform || 'cap',
          styles
        )
      )
      ...
      {character ? (
        <span
          style={{
            ...
            textDecoration: characterDisplayFormat?.styles?.textDecoration
          }}
        >
...
EventCharacterElement.displayName = 'EventCharacterElement'

export default EventCharacterElement
